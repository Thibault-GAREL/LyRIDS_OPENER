...
class BatchTripletMarginLoss(nn.Module):
    ...
    def forward(self, entity_types: torch.Tensor, embeddings: torch.Tensor) -> torch.Tensor:
        ...
        n_valid = valid_triplets.sum()
        loss = torch.clamp(triplet_distances + self._margin, 0.).sum()
        # Garde anti-NaN : si aucun triplet valide dans le batch (peu de labels +
        # petit batch), loss/0 = NaN -> corrompt le modele -> KMeans crash.
        if n_valid == 0:
            return loss * 0.0
        return loss / n_valid
...
class EntityTypingTrainer(BaseTrainer):
    ...
    def train(self):
        ...
        epoch = 0
        # Eval intermediaire (step 0 + par epoch) SUPPRIMEE : re-encoder tout le
        # test a chaque epoch coute enorme sur les gros test sets (jours). Seule
        # l'eval finale (evaluate()) compte pour le resultat.
    ...
    def train_one_epoch(self, epoch: int, train_dataloader: DataLoader, test_dataloader: DataLoader):
        ...
        for i, batch in tqdm(enumerate(train_dataloader), total=train_dataloader_len):
            ...
            if i % 20 == 0:  # log moins souvent (le file store ecrit a chaque appel -> overhead)
                mlflow.log_metrics({
                    'et_train_batch_triplet-margin-loss': loss.item()
                }, step=current_step)
        self.model.eval()
        # eval par epoch supprimee (cf. train()) -> gros gain de temps
