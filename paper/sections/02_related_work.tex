...
Early neural taggers replaced hand-crafted features with recurrent encoders over a CRF layer: Lample et al. \cite{lample2016ner} paired character- and word-level representations in a BiLSTM-CRF, and Ma and Hovy \cite{ma2016end} added a character-level CNN to obtain a fully end-to-end sequence labeler.
The pretrained-encoder paradigm subsequently raised in-domain accuracy further, fine-tuning a bidirectional transformer such as BERT \cite{devlin2019bert} with a lightweight classification head.
Beyond flat tagging, span-based formulations enumerate candidate spans and classify them directly, which simplifies the handling of nested and discontinuous entities \cite{nguyen2023span}: Yu et al. \cite{yu2020biaffine} score every span with a biaffine layer borrowed from dependency parsing, while Li et al. \cite{li2020mrc} recast typing as machine reading comprehension, querying the sentence with a natural-language description of each type -- a use of label semantics our zero-shot head also exploits.
These models reach high accuracy on in-domain benchmarks such as CoNLL-2003 \cite{tjongkimsang2003conll}, as surveyed comprehensively by Li et al. \cite{li2022survey}, but their label set is frozen at training time.
...
Zero-shot typing has also been driven by the semantics of the type names themselves: Aly et al. \cite{aly2021zeroshot} were the first to leverage natural-language type \emph{descriptions} so that classes unseen at training time can still be recognised, a premise our label-name prototypes share.
...
The choice of base encoder is made empirically: we compare several strong open sentence encoders (E5 \cite{wang2022e5}, BGE \cite{xiao2023cpack}, MPNet \cite{song2020mpnet} and mxbai \cite{li2024aoe}) on entity typing under a common protocol (\autoref{tab:ablation}). Nomic v1.5 \cite{nussbaum2024nomic} edges them out while additionally offering native Matryoshka truncation, so we adopt it as the base and fine-tune it.
...
Henderson et al. \cite{henderson2020systematic} argue for the systematic reporting of energy and carbon and provide a framework to track them per run, while Wu et al. \cite{wu2022sustainable} take a whole-lifecycle, system-level view of AI's environmental footprint across data, algorithms and hardware.
Most relevant to deployment, Luccioni et al. \cite{luccioni2024power} measure \emph{inference}-time cost and find general-purpose generative models to be orders of magnitude more expensive per query than task-specific ones -- precisely the gap a lightweight pipeline like ours seeks to close.
