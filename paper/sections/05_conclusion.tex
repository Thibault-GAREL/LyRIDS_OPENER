...
The quality scores themselves are nonetheless well characterised: retraining the full embedder under three seeds leaves the benchmark means nearly unchanged (std of $0.1$--$1.3$ AMI on the thirteen-set means, \autoref{sec:exp:protocol}), inference is deterministic at a fixed seed, and a test-set bootstrap confirms per-dataset stability (95\% CIs of $\pm 1$--$1.5$ AMI, \autoref{tab:bootstrap}); only latency and energy fluctuate ($\pm 15\%$) and are therefore read as averages rather than precise points.
A larger compute budget would extend the same protocol to the full uncapped test splits and a controlled-temperature re-measurement of latency and energy.
