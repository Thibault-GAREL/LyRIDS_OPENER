...
Second, the three-axis evaluation locates the open-world bottleneck in \emph{detection} rather than typing---end-to-end quality tracks the detector's recall on specialised domains---and exposes efficiency gaps that an accuracy-only comparison would hide.
...
Finally, all results use a single random seed, and energy is measured on one thermally-throttling consumer GPU, so per-run figures carry run-to-run variance and are read as averages rather than precise points.
