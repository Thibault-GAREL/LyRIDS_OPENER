...
GLiNER-L & 42.9 & 48.9 & \textbf{58.0} & 49.8 & 52.8 & 28.9 & 30.9 & \textbf{37.6} & 28.1 & 34.6 & 29.7 & 32.7 & 30.5 & 38.9 \\
...
% --- end preserved ---
OPENER & 41.0 & 47.0 & 53.3 & 48.3 & 50.0 & 24.0 & \textbf{33.1} & 37.3 & \textbf{30.1} & 33.5 & 45.6$^\dagger$ & 33.2 & 33.9 & \textbf{39.2} \\
\bottomrule
\end{tabular}
\end{table}
...
OPENER & 162 & 165 & 170 & 333 & 292 & 187 & 169 & 86 & 86 & 103 & 106 & 122 & 116 & 161 \\
\bottomrule
\end{tabular}
\end{table}
...
OPENER & 1.69 & 1.38 & 1.72 & 2.89 & 1.80 & 1.37 & 1.86 & 1.65 & 1.58 & 2.01 & 1.96 & 2.31 & 2.24 & 1.88 \\
\bottomrule
\end{tabular}
\end{table}
...
OPENER    & 467M & 39.2 & 161 & 1.88 & 0.10 \\
\bottomrule
\end{tabular}
\end{table}
...
Among embedder variants, an error-driven \emph{hard-negative mining} pass---fine-tuning the contrastive embedder for two epochs on the entity pairs it most often confuses (e.g.\ \emph{writer}/\emph{person}, \emph{album}/\emph{band}, \emph{country}/\emph{location}), mixed with correctly-typed mentions for diversity---raises typing-on-gold AMI from $54.0$ to $60.1$ ($+6.1$, better on $11$ of $13$ sets) and is our selected embedder configuration.
...
\begin{table}[tbp]
\centering
\caption{OPENER architecture search. One component is varied at a time around the default pipeline (\textbf{bold}). AMI and macro-$F_1$ ($\times100$, $\uparrow$); per-run latency, energy and carbon ($\downarrow$). $^\ast$\,entity-embedder rows are typing-on-gold over all $13$ sets; \emph{hard-negative mining} fine-tunes the contrastive embedder a \emph{second} time on the mention pairs it most often confuses (hard negatives in the sense of \cite{schroff2015facenet}), mixed with correctly-typed mentions for diversity ($3000$ triplets, $65\%$ hard, $2$ epochs); errors are mined on each training split by cross-validation, so no test label is seen. $^\ddagger$\,entity-typing sweep on a 10-dataset subset with the contrastive embedder; $^\S$\,dimension sweep on a 6-dataset pilot with the frozen embedder. Latency and energy are dominated by the embedder and detector, hence near-constant across classifiers; per-axis efficiency for the current pipeline is being re-measured.}
\label{tab:ablation}
\small
...
\quad \textbf{GLiNER-L}              & -- & -- & -- & -- & -- \\
\addlinespace
\multicolumn{6}{l}{\emph{Entity embedder}$^{\ast}$} \\
\quad frozen Nomic v1.5                       & -- & -- & -- & -- & -- \\
\quad contrastive (Triplet, CoNLL, 3\,ep.)    & 54.0 & 63.3 & -- & -- & -- \\
\quad \textbf{+ hard-negative mining (2\,ep., 3k\,tr.)} & \textbf{60.1} & \textbf{68.8} & -- & -- & -- \\
\addlinespace
...
OPENER & \textbf{57.3} & \textbf{64.7} & \textbf{71.1} & \textbf{72.5} & \textbf{67.2} & \textbf{40.1} & \textbf{59.5} & \textbf{61.1} & \textbf{50.7} & \textbf{64.7} & 70.6$^\dagger$ & \textbf{48.6} & \textbf{53.1} & \textbf{60.1} \\
\bottomrule
\end{tabular}
\end{table}
