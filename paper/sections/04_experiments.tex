...
Its un-refined nearest-centroid baseline, OPENER-ZS (base), already matches the zero-shot OWNER baseline end-to-end ($34.2$ vs $34.3$ mean AMI) and edges it on gold ($44.4$ vs $43.0$, \autoref{tab:et-gold}), while keeping OPENER's frugal inference profile ($137$\,ms, $1.85$\,Wh vs $616$\,ms, $14.3$\,Wh).
Two label-free refinements then build the full OPENER-ZS: a transductive k-means on the prototypes, and \emph{fusing in the detector's own zero-shot label predictions}      --      a signal the centroid otherwise discards      --      which together reach $39.4$ end-to-end, the best of all compared zero-shot systems, ahead of GLiNER-L ($38.9$) and within $0.8$ AMI of OPENER-Sup, all without any target label.
...
\rowcolor{hlyellow} OPENER-ZS & 39.4 & 48.8 & \textbf{58.0} & 48.6 & 52.8 & 29.6 & 31.5 & 35.5 & 27.8 & 35.7 & 43.1$^\dagger$ & 31.2 & 30.4 & 39.4 \\
...
\legend{GLiNER-S, GLiNER-M, GLiNER-L, GNER, Qwen2.5-1.5B, OWNER, OPENER-ZS, OPENER-Sup}
\end{axis}
\end{tikzpicture}
...
\rowcolor{hlyellow} OPENER-ZS & 218 & 254 & 291 & 277 & 289 & 185 & 173 & 205 & 84 & 98 & 92 & 104 & 97 & 182 \\
% MOVED to architecture search (\autoref{tab:ablation}); preserved e2e per-dataset:
...
\rowcolor{hlyellow} OPENER-ZS & 1.14 & 1.23 & 1.58 & 2.08 & 1.82 & 1.41 & 1.82 & 2.23 & 1.62 & 1.87 & 1.63 & 1.90 & 1.79 & 1.70 \\
% MOVED to architecture search (\autoref{tab:ablation}); preserved e2e per-dataset:
...
\rowcolor{hlyellow} OPENER-ZS & 467M & 39.4 & 182 & 1.70 & 0.10 \\
...
\pgfplotsset{
  metricbar/.style={
    ...
    xticklabels={GL-S,GL-M,GL-L,GNER,Qwen,OWN,ZS$_0$,ZS,Sup},
...
First, the detector-fusion gains of OPENER-ZS concentrate on the context-rich encyclopedic CrossNER domains ($+16.3$ AMI on Music, $+11.9$ on Politics, $+8.6$ on Science over the un-fused centroid), where the detector's own zero-shot label is most reliable and lifts OPENER-ZS to detector-level quality (e.g.\ Music $58.0$, matching GLiNER-L).
...
\begin{table}[tbp]
\centering
\caption{Entity typing on gold mentions (detection bypassed): OPENER vs OWNER (AMI $\times 100$, $\uparrow$). Abbreviations follow \autoref{tab:datasets}; bold marks the best value per column; systems are grouped by supervision. Here OPENER-ZS is the transductively-refined prototype head (the detector fusion of \autoref{tab:main} is end-to-end-only, inactive without a detector); OPENER-ZS (base) is the un-refined nearest centroid; OPENER-Sup is the supervised upper bound. $^\dagger$\,CoNLL-2003 is in-domain for OWNER (training source) and OPENER (contrastive source) and is not bolded.}
\label{tab:et-gold}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{l*{13}{c}c}
...
\rowcolor{hlyellow} OPENER-ZS & 46.4 & 65.0 & 68.6 & 66.5 & 57.3 & 31.8 & 52.1 & 45.8 & 37.5 & 49.5 & 71.0$^\dagger$ & 38.6 & 37.5 & 51.4 \\
\addlinespace
\multicolumn{15}{l}{\emph{Supervised (target labels)}} \\
