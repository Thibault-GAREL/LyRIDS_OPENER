...
% Non-blind: exact hardware disclosed (NVIDIA GTX 1660 Ti Max-Q, 6 GB VRAM).
...
\item[] Protocol, which differs per axis. On the \emph{mention-detector} axis each row is OPENER's \emph{end-to-end} AMI with that detector, which only proposes spans that the OPENER head re-types (unlike the standalone systems of \autoref{tab:main}). The $^\ast$\,embedder, $^\ddagger$\,typing and $^\S$\,dimension axes are instead \emph{typing-on-gold} over the $13$ sets. \item[] Cost. Latency and energy are the deployed end-to-end cost on the mention-detector axis, and the constant embedder-plus-typing cost on the other axes. \item[$\S$] Truncates the Matryoshka vector after encoding.\item[$\P$] Zero-shot nearest-centroid head (no target labels). Its Fit is the one-off prototype-embedding cost.
\end{tablenotes}
\end{threeparttable}
\end{table}
