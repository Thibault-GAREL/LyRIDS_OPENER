...
\begin{table}[t]
\centering
\caption{Test-set bootstrap robustness of the released (seed-$42$) model on three representative datasets, for OPENER-Sup and the inductive zero-shot head \zsind{} (the transductive refinement is excluded, as it adapts to the resampled test distribution). Cells report typing-on-gold AMI as bootstrap mean $\pm$ std with the $95\%$ percentile CI ($K{=}1000$ resamples of the gold mentions). Point estimates on the full test set are $71.5 / 71.8 / 59.2$ (Sup) and $65.9 / 55.2 / 35.6$ (\zsind), matching the bootstrap means. Intervals of $\pm 1$--$1.5$ AMI indicate stable scores.}
\label{tab:bootstrap}
\small
\begin{tabular}{lcc}
\toprule
