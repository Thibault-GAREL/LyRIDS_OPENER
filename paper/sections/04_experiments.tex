...
We evaluate on \textbf{13 evaluation sets} that span news, social media, spoken queries, encyclopedic text, biomedical abstracts and manufacturing literature, following the dataset selection of OWNER \cite{genest2025owner}.
...
Predicted and gold mentions are aligned by exact character offsets, and any unmatched mention receives a distinct sentinel label so that detection errors are penalised rather than ignored, following the sentinel-based scoring detailed in \autoref{sec:method:problem}.
...
Every OPENER cell in the result tables therefore reports the mean\,$\pm$\,standard deviation over the three seeds. Seed $42$ is the released model.
...
We additionally quantify the test-set sampling uncertainty with a bootstrap on the released model (\autoref{tab:bootstrap}), a source of uncertainty orthogonal to the retraining seed.
...
Because inference is deterministic, the only remaining source of uncertainty in the quality scores is the finite test set. We quantify it with a non-parametric bootstrap: for three representative datasets we resample the gold mentions with replacement ($K{=}1000$ resamples) and recompute typing-on-gold AMI, yielding the $95\%$ confidence intervals in \autoref{tab:bootstrap}. The intervals are narrow ($\pm 1$ to $1.5$ AMI) and their point estimates coincide with the released model's full-test-set scores (given in the caption of \autoref{tab:bootstrap}), indicating that the reported figures are stable rather than fragile, seed-specific artefacts.
\autoref{tab:hyperparams} collects the full OPENER configuration used throughout. The embedder is retrained under each of the three seeds ($7$, $42$, $123$) while every other component is fixed.

\begin{table}[t]
\centering
\caption{Test-set bootstrap robustness of the released (seed-$42$) model on three representative datasets, for OPENER-Sup and the inductive zero-shot head \zsind{} (the transductive refinement is excluded, as it adapts to the resampled test distribution). Cells report typing-on-gold AMI as bootstrap mean $\pm$ std with the $95\%$ percentile CI ($K{=}1000$ resamples of the gold mentions). Point estimates on the full test set are $71.5 / 71.8 / 59.2$ (Sup) and $65.9 / 55.2 / 35.6$ (\zsind), matching the bootstrap means. Intervals of $\pm 1$ to $1.5$ AMI indicate stable scores.}
\label{tab:bootstrap}
\small
\begin{tabular}{lcc}
\toprule
Dataset & OPENER-Sup AMI & \zsind{} AMI \\
\midrule
...
\begin{table}[t]
\centering
\caption{OPENER configuration. The detector and embedder are ready-to-use. The contrastive and hard-mining stages produce the shared embedder. The typing head is fitted (supervised) or label-free (zero-shot).}
...
\begin{table}[H]
\centering
\caption{Open-world NER, end-to-end. AMI ($\times 100$, $\uparrow$). Abbreviations follow \autoref{tab:datasets}. Bold marks the best value per column across all systems. Systems are grouped by supervision (\emph{zero-shot} vs the \emph{supervised} OPENER-Sup). OPENER rows report mean\,$\pm$\,std over the three embedder-retraining seeds (\autoref{sec:exp:protocol}). The baselines are frozen checkpoints with deterministic inference (single run). $^\dagger$\,CoNLL-2003 is in-domain for OWNER (training source) and OPENER (contrastive source) and is not bolded as a transfer result. $^\ddagger$\,Qwen2.5-1.5B-Instruct (4-bit) is on a $150$-sentence subsample (indicative, not bolded). Typing-on-gold is reported in \autoref{tab:et-gold}.}
\label{tab:main}
\footnotesize
\setlength{\tabcolsep}{4pt}
...
\begin{table}[H]
\centering
\caption{Quality vs efficiency, $13$-set means. AMI ($\times 100$, $\uparrow$, OPENER rows: mean\,$\pm$\,std over three retraining seeds). P50 latency (ms), energy (Wh), carbon (gCO\textsubscript{2}eq) per run ($\downarrow$, measured on the released seed-$42$ run). Parameters (end-to-end OPENER $=$ GLiNER-L detector $330$M $+$ embedder $137$M $=$ $467$M, or $137$M on gold, OWNER $\approx$294M, GLiNER sizes from project notes). $^\S$\,OWNER's latency is an amortised wall-clock mean and its one-time $88$-min training is not charged per run (\autoref{tab:latency}). $^\ddagger$\,Qwen2.5-1.5B-Instruct (4-bit) is on a $150$-sentence subsample per set.}
...
First, the detector-fusion gains of \zstr concentrate on the context-rich encyclopedic CrossNER domains ($+14.7$ AMI on Music, $+10.1$ on Science, $+8.8$ on Politics over the un-fused centroid), where the detector's own zero-shot label is most reliable and lifts OPENER-ZS to detector-level quality (e.g.\ Music $58.3$, edging GLiNER-L's $58.0$).
Second, on specialised schemas with opaque, abbreviation-like labels (FabNER, BioNLP), OPENER's contrastive typing maps cryptic surface forms far better than OWNER's prompt-based naming: OPENER-Sup leads end-to-end ($29.8$ and $35.9$ AMI), and on gold mentions the gap widens sharply ($57.1$/$69.2$ versus OWNER's $31.9$/$29.2$, \autoref{tab:et-gold}).
...
For OPENER-Sup, the mean drop from gold to end-to-end is $22.2$ AMI, but it is far from uniform: it averages $18.8$ on the encyclopedic CrossNER domains against $29.9$ on the specialised schemas (BioNLP, MIT-Restaurant, MIT-Movie, FabNER), peaking at $33.3$ on BioNLP.
The typing head is strong everywhere (gold AMI above $50$ on $11$ of $13$ sets). What caps end-to-end quality is the open-world detector's recall on cryptic, domain-specific surface forms, which confirms detection, not typing, as the dominant bottleneck.
...
\begin{table}[tbp]
\centering
\caption{Entity typing on gold mentions (detection bypassed): OPENER vs OWNER (AMI $\times 100$, $\uparrow$). Abbreviations follow \autoref{tab:datasets}. Bold marks the best value per column. Systems are grouped by supervision. OPENER rows report mean\,$\pm$\,std over the three embedder-retraining seeds (\autoref{sec:exp:protocol}). OWNER is a single training run. Here \zstr is the transductively-refined prototype head (the detector fusion of \autoref{tab:main} is end-to-end-only, inactive without a detector). \zsind is the un-refined nearest centroid. OPENER-Sup is the supervised upper bound. $^\dagger$\,CoNLL-2003 is in-domain for OWNER (training source) and OPENER (contrastive source) and is not bolded.}
\label{tab:et-gold}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{l*{13}{c}c}
...
\begin{table}[tbp]
\centering
\caption{Zero-shot typing: effect of the contrastive embedder on \zsind (typing-on-gold, $13$-set means at the released seed ($42$). The nearest label-name centroid head is fixed, only the embedder changes). AMI, macro-$F_1$, accuracy ($\times100$, $\uparrow$). Best per column in bold. The selected hard-negative-mining embedder (target-tuned) is compared with Pile-NER \cite{zhou2024universalner} variants that are strictly domain-agnostic (never see target data). We report Pile-NER as a negative ablation (\autoref{sec:exp:ablations}).}
...
Attention bien mettre la conclusion que le matryoshka vector déscente de façon linéaire, et que la taille du vecteur est un compromis entre coût et qualité. il faut donc dire que malgré notre entrainement contrastif, l'embedder n'a pas perdu sa cacité de Matryoshka.
