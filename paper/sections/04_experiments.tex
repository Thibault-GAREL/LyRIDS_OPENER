...
\footnotesize
\item ``Test'' is the number of test sentences used (capped at 1000). Each type is followed by its share (\%) of the test mentions, listed by decreasing frequency (types are the dataset's original labels).
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{tablenotes}[flushleft]
\footnotesize
\item ``Type'' is the transformer family. ``vs OPENER'' is the total parameter count relative to OPENER's end-to-end pipeline ($467$M $=$ GLiNER-L $330$M detector $+$ Nomic $137$M embedder).
...
\item \textbf{Qwen2.5-1.5B-Instruct} \cite{qwen2025}: the only generative LLM that fits our $6$\,GB GPU, runs in 4-bit NF4 \cite{dettmers2023qlora} and prompted for JSON-formatted entities. Even at this small size, its autoregressive decoding makes it the slowest and most energy-hungry system we measure, showing that generative extraction remains costly regardless of model size.
...
\item[$\ddagger$] Qwen2.5-1.5B-Instruct (4-bit) is on a $150$-sentence subsample (indicative, not bolded).
\end{tablenotes}
\end{threeparttable}
\end{table}
\begin{figure}[H]
