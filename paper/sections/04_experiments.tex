...
Because OWNER retrains one model per dataset, its cost on our 6\,GB GPU is high; to cover every set, the slowest seven were trained for a single epoch (cells marked $\ddagger$).
Entity typing converges quickly, so the typing-on-gold scores remain robust; the end-to-end scores on the single-epoch sets are a lower bound, as the detector is undertrained.
\emph{[Narrative analysis to be finalised.]}

\begin{table}[tbp]
\centering
\caption{Open-world NER, end-to-end protocol. Adjusted Mutual Information (AMI $\times 100$, $\uparrow$). Abbreviations follow \autoref{tab:datasets}; bold marks the best system per column. $^\ddagger$\,On these 7 sets OWNER's detector was trained for a single epoch (compute budget); its end-to-end score there is a lower bound, as the detector is undertrained. The typing-on-gold comparison is reported in \autoref{tab:et-gold}.}
\label{tab:main}
\footnotesize
...
OWNER & 3.9$^\ddagger$ & 23.0$^\ddagger$ & 45.4 & \textbf{53.5} & 53.5 & 29.9 & 36.7$^\ddagger$ & 40.5$^\ddagger$ & 33.8$^\ddagger$ & 35.4 & 49.0 & 28.3$^\ddagger$ & 26.7$^\ddagger$ & 35.4 \\
...
\begin{table}[tbp]
\centering
\caption{Energy. Energy per run in watt-hours ($\downarrow$), end-to-end. Abbreviations follow \autoref{tab:datasets}; best per column in bold. $^\ddagger$\,OWNER's energy mixes 4-epoch and 1-epoch runs (cf.\ \autoref{tab:main}); it trains one model per dataset, hence the high cost.}
\label{tab:energy}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{l*{13}{c}c}
...
OWNER$^\ddagger$ & 18.91 & 21.42 & 47.97 & 90.54 & 77.83 & 275.15 & 109.97 & 191.68 & 180.80 & 559.75 & 392.76 & 146.30 & 144.41 & 173.65 \\
...
OWNER$^\ddagger$ & 294M & 35.4$^\ddagger$ & -- & 173.65 & 9.03 \\
...
\begin{table}[tbp]
\centering
\caption{Entity typing on gold mentions: OPENER vs OWNER (AMI $\times 100$, $\uparrow$). Detection is bypassed, isolating the typing component. Abbreviations follow \autoref{tab:datasets}; bold marks the per-dataset winner. OWNER leads on $10$ of $13$ sets. $^\ddagger$\,GUM (and the OWNER average) include one dataset trained for a single epoch; typing converges fast, so the effect is small here.}
\label{tab:et-gold}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{l*{13}{c}c}
...
Model & AI & Lit & Mus & Pol & Sci & WNUT & Rest & Movie & Fab & Bio & CoNLL & GUM & GEN & Avg \\
\midrule
OWNER$^\ddagger$ & \textbf{60.8} & \textbf{59.7} & \textbf{65.2} & 65.5 & \textbf{68.1} & \textbf{42.1} & \textbf{62.8} & \textbf{64.0} & \textbf{57.9} & 52.3 & 50.5 & \textbf{47.2}$^\ddagger$ & \textbf{49.9} & \textbf{57.4} \\
OPENER & 55.6 & 53.5 & 64.2 & \textbf{66.6} & 66.1 & 41.8 & 51.2 & 54.2 & 36.5 & \textbf{60.2} & \textbf{70.7} & 37.7 & 44.2 & 54.0 \\
\bottomrule
\end{tabular}
\end{table}
