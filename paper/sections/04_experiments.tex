...
\begin{table}[H]
\centering
\caption{The 13 evaluation sets and their gold entity types. ``Test'' is the number of test sentences used (capped at 1000); each type is followed by its share (\%) of the test mentions, listed by decreasing frequency (types are the dataset's original labels).}
\label{tab:datasets}
\scriptsize
...
\begin{table}[H]
\centering
\caption{Backbones and size of the re-measured baselines and OPENER, in the spirit of Genest et al.\ \cite{genest2025owner}. ``Type'' is the transformer family; ``vs OPENER'' is the total parameter count relative to OPENER's end-to-end pipeline ($467$M $=$ GLiNER-L $330$M detector $+$ Nomic $137$M embedder). $^\dagger$\,OWNER reports $110$M for its BERT typing encoder alone; we count detection $+$ typing for parity with OPENER.}
\label{tab:baselines}
\footnotesize
...
OWNER$^\dagger$ & DeBERTaV3+BERT & Encoder & deberta-v3 + bert-base & 294M & 0.6$\times$ \\
Qwen2.5-1.5B & Qwen2.5 & Decoder & qwen2.5-1.5b-instruct & 1.5B & 3.2$\times$ \\
\midrule
\textbf{OPENER} (ours) & DeBERTaV3+Nomic & Encoder & deberta-v3-large + nomic-v1.5 & 467M & 1.0$\times$ \\
\bottomrule
\end{tabular}
\end{table}

We compare OPENER against representative open-world systems, all run end-to-end on the same splits and scored with the same protocol (\autoref{sec:exp:protocol}); \autoref{tab:baselines} summarises their backbones and sizes.
\begin{itemize}
\item \textbf{GLiNER} \cite{zaratiana2024gliner}: an open-world span classifier on a DeBERTaV3 encoder \cite{he2023debertav3}; we run all three sizes (S/M/L) to trace a size/efficiency trade-off.
\item \textbf{GNER} \cite{ding2024gner}: a generative tagger built on T5 \cite{raffel2020t5}; we run the T5-base checkpoint.
\item \textbf{OWNER} \cite{genest2025owner}: our closest baseline, a DeBERTaV3 detector with a BERT typing encoder \cite{devlin2019bert} clustered into dynamically named types; executed in its original environment.
\item \textbf{Qwen2.5-1.5B-Instruct} \cite{qwen2025}: the only generative LLM that fits our $6$\,GB GPU, run in 4-bit NF4 \cite{dettmers2023qlora} and prompted for JSON-formatted entities.
\item \textbf{Large LLM systems}: UniversalNER \cite{zhou2024universalner}, GoLLIE \cite{sainz2024gollie} and ChatIE \cite{wei2024chatie} rely on $7$B-parameter backbones \cite{touvron2023llama} that exceed our hardware even under 4-bit quantization; we report the figures published by their authors rather than re-measuring them, and omit them from \autoref{tab:baselines}.
\end{itemize}
...
All runs use a single fixed random seed ($42$). Since no stage of OPENER is stochastic at inference (\autoref{sec:exp:impl}), the reported quality metrics carry no run-to-run noise; we nonetheless quantify their sampling uncertainty with a test-set bootstrap (\autoref{tab:bootstrap}) and leave a full multi-seed \emph{retraining} study to future work, as it is costly on our $6$\,GB GPU.
...
All systems are measured on the same machine. The consumer GPU is subject to \emph{thermal throttling} (it lowers its clock frequency under sustained load to stay within its temperature limit), so per-sentence latency and per-run energy carry roughly $\pm15\%$ run-to-run variance; we therefore read the per-system averages as indicative and do not over-interpret individual per-dataset cells. A controlled-temperature re-measurement of all systems in a single session is left to future work.
...
Because inference is deterministic, the only remaining source of uncertainty in the quality scores is the finite test set. We quantify it with a non-parametric bootstrap: for three representative datasets we resample the gold mentions with replacement ($K{=}1000$ resamples) and recompute typing-on-gold AMI, yielding the $95\%$ confidence intervals in \autoref{tab:bootstrap}. The intervals are narrow ($\pm 1$--$1.5$ AMI) and their point estimates coincide with the seed-$42$ scores reported above, indicating that the reported figures are stable rather than fragile, seed-specific artefacts.
\autoref{tab:hyperparams} collects the full OPENER configuration used throughout; all stochastic components use the single fixed seed ($42$).

\begin{table}[t]
\centering
\caption{Test-set bootstrap robustness of the released (seed-$42$) model on three representative datasets. Cells report typing-on-gold AMI as bootstrap mean $\pm$ std with the $95\%$ percentile CI ($K{=}1000$ resamples of the gold mentions). Point estimates on the full test set are $71.5 / 71.8 / 59.2$ (Sup) and $65.9 / 55.2 / 35.6$ (ZS), matching the bootstrap means. Intervals of $\pm 1$--$1.5$ AMI indicate stable scores.}
\label{tab:bootstrap}
\small
\begin{tabular}{lcc}
\toprule
...
\midrule
CoNLL-2003     & $71.7 \pm 1.5$ ~[$68.9$, $74.6$] & $66.1 \pm 1.4$ ~[$63.5$, $69.0$] \\
CrossNER-Music & $72.4 \pm 0.9$ ~[$70.6$, $74.2$] & $55.5 \pm 0.9$ ~[$53.8$, $57.3$] \\
FabNER         & $59.8 \pm 0.9$ ~[$57.9$, $61.6$] & $36.2 \pm 0.7$ ~[$34.7$, $37.5$] \\
\bottomrule
\end{tabular}
\end{table}
...
Concretely, excluding CoNLL-2003 (in-domain for both OWNER and OPENER), OWNER's reported $34.3$ thirteen-set mean becomes a $32.9$ transfer mean, still well below \zstr ($39.1$ on the same $12$ sets); the margin in fact widens from $+5.1$ to $+6.2$ AMI under this fair transfer reading.
...
Its un-refined nearest-centroid baseline, \zsind, already matches the zero-shot OWNER baseline end-to-end ($34.2$ vs $34.3$ mean AMI) and edges it on gold ($44.4$ vs $43.0$, \autoref{tab:et-gold}), while keeping OPENER's frugal inference profile ($137$\,ms, $1.85$\,Wh vs $616$\,ms, $14.3$\,Wh).
Two label-free refinements then build \zstr: a transductive k-means on the prototypes, and \emph{fusing in the detector's own zero-shot label predictions}      --      a signal the centroid otherwise discards      --      which together reach $39.4$ end-to-end, the best of all compared zero-shot systems, ahead of GLiNER-L ($38.9$) and within $0.8$ AMI of OPENER-Sup, all without any target label.
For reference, the only generative LLM that fits our $6$\,GB GPU      --      Qwen2.5-1.5B-Instruct \cite{qwen2025} in 4-bit, prompted for JSON-formatted entities      --      trails every dedicated system ($26.1$ mean AMI on a $150$-sentence subsample, \autoref{tab:main}) while being the slowest and most energy-hungry per sentence ($5.1$\,s, $11.4$\,Wh; \autoref{tab:efficiency}): a small open LLM pays on both axes at once.
OPENER-Sup remains the stronger configuration ($40.2$ end-to-end, $62.5$ on gold), so OPENER-ZS is best read as the frugal, annotation-free operating point of the same pipeline.
...
\begin{table}[H]
\centering
\caption{Open-world NER, end-to-end. AMI ($\times 100$, $\uparrow$); abbreviations follow \autoref{tab:datasets}; bold marks the best value per column across all systems; systems are grouped by supervision (\emph{zero-shot} vs the \emph{supervised} OPENER-Sup). $^\dagger$\,CoNLL-2003 is in-domain for OWNER (training source) and OPENER (contrastive source) and is not bolded as a transfer result. $^\ddagger$\,Qwen2.5-1.5B-Instruct (4-bit) is on a $150$-sentence subsample (indicative, not bolded). Typing-on-gold is reported in \autoref{tab:et-gold}.}
\label{tab:main}
\footnotesize
\setlength{\tabcolsep}{4pt}
...
\zsind & 31.6 & 43.8 & 41.7 & 36.7 & 44.2 & 23.0 & 28.4 & 32.4 & 28.3 & 29.1 & 42.6$^\dagger$ & 31.5 & 31.2 & 34.2 \\
\rowcolor{hlyellow} \zstr & 39.4 & 48.8 & \textbf{58.0} & 48.6 & 52.8 & 29.6 & 31.5 & 35.5 & 27.8 & 35.7 & 43.1$^\dagger$ & 31.2 & 30.4 & 39.4 \\
...
\rowcolor{hlred} OPENER-Sup & 40.2 & 49.6 & 55.3 & 50.3 & 51.7 & 24.9 & \textbf{33.6} & \textbf{37.8} & \textbf{30.2} & \textbf{36.5} & 44.8$^\dagger$ & 33.7 & 34.2 & \textbf{40.2} \\
\bottomrule
\end{tabular}
\end{table}
\begin{figure}[H]
...
\addplot[figgold!90!black, very thick, mark=square*, mark size=1.6pt] coordinates
  {(AI,39.4)(Lit,48.8)(Mus,58.0)(Pol,48.6)(Sci,52.8)(WNUT,29.6)(Rest,31.5)(Movie,35.5)(Fab,27.8)(Bio,35.7)(CoNLL,43.1)(GUM,31.2)(GEN,30.4)};
%       --       supervised (dashed)       --
\addplot[figred, very thick, densely dashed, mark=*, mark size=1.6pt] coordinates
  {(AI,40.2)(Lit,49.6)(Mus,55.3)(Pol,50.3)(Sci,51.7)(WNUT,24.9)(Rest,33.6)(Movie,37.8)(Fab,30.2)(Bio,36.5)(CoNLL,44.8)(GUM,33.7)(GEN,34.2)};
\legend{GLiNER-S, GLiNER-M, GLiNER-L, GNER, Qwen2.5-1.5B, OWNER, \zstr, OPENER-Sup}
\end{axis}
\end{tikzpicture}
\caption{End-to-end AMI per dataset (\autoref{tab:main}). \emph{Zero-shot} systems are drawn solid; OPENER-Sup (the \emph{supervised} upper bound) is the dashed red line. $^\dagger$\,CoNLL-2003 (shaded) is in-domain for OWNER and OPENER; $^\ddagger$\,Qwen is evaluated on a $150$-sentence subsample per set.}
\label{fig:main}
\end{figure}

\begin{table}[H]
\centering
\caption{Median per-sentence latency (ms, $\downarrow$), end-to-end; abbreviations follow \autoref{tab:datasets}, best per column in bold. $^\S$\,OWNER exposes no per-sentence call: its figure is an amortised wall-clock mean (its $88$-min training is excluded); $^\dagger$\,CoNLL is OWNER's in-domain source, measured by a separate inference-only run; $^\ddagger$\,Qwen2.5-1.5B-Instruct (4-bit) is on a $150$-sentence subsample per set.}
\label{tab:latency}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{l*{13}{c}c}
...
\begin{table}[H]
\centering
\caption{Energy consumption per run (Wh, $\downarrow$), end-to-end. Abbreviations follow \autoref{tab:datasets}; best per column in bold. $^\S$\,OWNER is measured at inference only (its one-time $88$-min CoNLL-2003 training is amortised, not charged per run); $^\dagger$\,CoNLL is its in-domain training source, measured by a separate inference-only run; $^\ddagger$\,Qwen2.5-1.5B-Instruct (4-bit) is on a $150$-sentence subsample per set.}
\label{tab:energy}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{l*{13}{c}c}
...
\begin{table}[H]
\centering
\caption{Quality vs efficiency, $13$-set means. AMI ($\times 100$, $\uparrow$); p50 latency (ms), energy (Wh), carbon (gCO\textsubscript{2}eq) per run ($\downarrow$); parameters (end-to-end OPENER $=$ GLiNER-L detector $330$M $+$ embedder $137$M $=$ $467$M, or $137$M on gold; OWNER $\approx$294M; GLiNER sizes from project notes). $^\S$\,OWNER's latency is an amortised wall-clock mean and its one-time $88$-min training is not charged per run (\autoref{tab:latency}). $^\ddagger$\,Qwen2.5-1.5B-Instruct (4-bit) is on a $150$-sentence subsample per set.}
...
\zsind & 467M & 34.2 & 137 & 1.85 & 0.10 \\
\rowcolor{hlyellow} \zstr & 467M & 39.4 & 182 & 1.70 & 0.10 \\
...
\rowcolor{hlred} OPENER-Sup & 467M & 40.2 & 143 & 1.59 & 0.09 \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\begin{tikzpicture}
\begin{axis}[metricbar, title={AMI ($\uparrow$)}, ymin=0, ymax=45, ytick={0,15,30,45}]
...
\addplot[ybar,bar shift=0pt,fill=figgold!55!black,draw=figgold!40!black] coordinates {(7,34.2)};
\addplot[ybar,bar shift=0pt,fill=figgold!90!black,draw=figgold!60!black] coordinates {(8,39.4)};
\addplot[ybar,bar shift=0pt,fill=figred,draw=figred!65!black] coordinates {(9,40.2)};
\draw[densely dotted, semithick, black!55] (axis cs:8.5,0) -- (axis cs:8.5,45);
\end{axis}
...
% DRAFT TODO (not rendered): consider adding Pareto diagrams (AMI vs. latency,
% AMI vs. energy consumption) to visualise the frugality frontier. Decide before submission.
...
First, the detector-fusion gains of \zstr concentrate on the context-rich encyclopedic CrossNER domains ($+16.3$ AMI on Music, $+11.9$ on Politics, $+8.6$ on Science over the un-fused centroid), where the detector's own zero-shot label is most reliable and lifts OPENER-ZS to detector-level quality (e.g.\ Music $58.0$, matching GLiNER-L).
Second, on specialised schemas with opaque, abbreviation-like labels (FabNER, BioNLP), OPENER's contrastive typing maps cryptic surface forms far better than OWNER's prompt-based naming: OPENER-Sup leads end-to-end ($30.2$ and $36.5$ AMI), and on gold mentions the gap widens sharply ($59.2$/$71.2$ versus OWNER's $31.9$/$29.2$, \autoref{tab:et-gold}).
...
For OPENER-Sup, the mean drop from gold to end-to-end is $22.3$ AMI, but it is far from uniform: it averages $18.6$ on the encyclopedic CrossNER domains against $30.6$ on the specialised schemas (BioNLP, MIT-Restaurant, MIT-Movie, FabNER), peaking at $34.7$ on BioNLP.
The typing head is strong everywhere (gold AMI $50$--$75$); what caps end-to-end quality is the open-world detector's recall on cryptic, domain-specific surface forms, which confirms detection, not typing, as the dominant bottleneck.

\paragraph{What remains hard}
The residual typing errors are dominated by confusions between fine-grained, lexically close types \emph{within} a domain rather than across domains.
...
Frozen off-the-shelf, the Nomic embedding intermixes entity types; the contrastive step already pulls same-type mentions together, and the hard-negative mining pass sharpens the boundaries between the types it most often confuses, yielding compact, well-separated clusters.
...
\begin{table}[tbp]
\centering
\caption{OPENER architecture search: one component is varied at a time around the default pipeline (\textbf{bold}). AMI and macro-$F_1$ ($\times100$, $\uparrow$); Fit\,(ms) is the typing-head training time; per-row latency, energy and carbon ($\downarrow$). On the \emph{mention-detector} axis, each row is OPENER's \emph{end-to-end} AMI with that detector -- it only proposes spans, re-typed by the OPENER head (unlike the standalone systems of \autoref{tab:main}). The $^\ast$\,embedder, $^\ddagger$\,typing and $^\S$\,dimension axes are typing-on-gold over the $13$ sets ($^\S$ truncates the Matryoshka vector after encoding); $^\P$\,marks the zero-shot nearest-centroid head (no target labels; its Fit is the one-off prototype-embedding cost). Latency/energy is the deployed end-to-end cost on the MD axis and the constant embedder-plus-typing cost elsewhere; per-axis findings are analysed in \autoref{sec:exp:ablations}.}
\label{tab:ablation}
\small
...
\quad frozen bge-base-en-v1.5 \cite{xiao2023cpack}      & 23.6 & 41.9 & 0 & 43 & 0.63 & 0.04 \\
...
\begin{table}[tbp]
\centering
\caption{Entity typing on gold mentions (detection bypassed): OPENER vs OWNER (AMI $\times 100$, $\uparrow$). Abbreviations follow \autoref{tab:datasets}; bold marks the best value per column; systems are grouped by supervision. Here \zstr is the transductively-refined prototype head (the detector fusion of \autoref{tab:main} is end-to-end-only, inactive without a detector); \zsind is the un-refined nearest centroid; OPENER-Sup is the supervised upper bound. $^\dagger$\,CoNLL-2003 is in-domain for OWNER (training source) and OPENER (contrastive source) and is not bolded.}
\label{tab:et-gold}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{l*{13}{c}c}
...
\zsind & 41.4 & 61.4 & 55.2 & 50.7 & 58.0 & 33.0 & 42.6 & 18.4 & 35.6 & 54.1 & 65.9$^\dagger$ & 31.1 & 30.2 & 44.4 \\
\rowcolor{hlyellow} \zstr & 46.4 & 65.0 & 68.6 & 66.5 & 57.3 & 31.8 & 52.1 & 45.8 & 37.5 & 49.5 & 71.0$^\dagger$ & 38.6 & 37.5 & 51.4 \\
\addlinespace
\multicolumn{15}{l}{\emph{Supervised (target labels)}} \\
% hard-mining 3k tr./2 ep. (recorded in project notes): 57.3 64.7 71.1 72.5 67.2 40.1 59.5 61.1 50.7 64.7 70.6 48.6 53.1 | 60.1
\rowcolor{hlred} OPENER-Sup & \textbf{57.6} & \textbf{67.0} & \textbf{71.8} & \textbf{74.8} & \textbf{68.9} & \textbf{38.6} & \textbf{63.0} & \textbf{66.9} & \textbf{59.2} & \textbf{71.2} & 71.5$^\dagger$ & \textbf{50.3} & \textbf{51.5} & \textbf{62.5} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[tbp]
\centering
\caption{Zero-shot typing: effect of the contrastive embedder on \zsind (typing-on-gold, $13$-set means; the nearest label-name centroid head is fixed, only the embedder changes). AMI, macro-$F_1$, accuracy ($\times100$, $\uparrow$); best per column in bold. The selected hard-negative-mining embedder (target-tuned) is compared with Pile-NER \cite{zhou2024universalner} variants that are strictly domain-agnostic (never see target data); we report Pile-NER as a negative ablation (\autoref{sec:exp:ablations}).}
